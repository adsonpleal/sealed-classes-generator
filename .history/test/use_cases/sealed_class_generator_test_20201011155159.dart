...
void main() {
  ...
  test('should generate code if there are annotate classes', () {
    ...
    when(buildGeneratedCode(classes)).thenReturn(generatedCode);

    final actual = generator.generate(null, null);

    expect(actual, generatedCode);

    verify(getAnnotateClasses(any)).called(1);
    verify(buildGeneratedCode(classes)).called(1);
    verifyNoMoreInteractions(getAnnotateClasses);
    verifyNoMoreInteractions(buildGeneratedCode);
  });
}
