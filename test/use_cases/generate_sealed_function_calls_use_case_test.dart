...
void main() {
  ...
  test('should generate function call without else branch', () {
    const functionCall1 = 'functionCall1';
    const functionCall2 = 'functionCall2';
    final constructor1 = ConstructorElementMock();
    final constructor2 = ConstructorElementMock();
    ...
    when(constructorAsFunctionCall(constructor1, false))
        .thenReturn(functionCall1);
    when(constructorAsFunctionCall(constructor2, false))
        .thenReturn(functionCall2);

    final actual = useCase(constructors);

    expect(
      actual,
      "$functionCall1 else $functionCall2",
    );
    verify(constructorAsFunctionCall(constructor1, false)).called(1);
    verify(constructorAsFunctionCall(constructor2, false)).called(1);
    verifyNoMoreInteractions(constructorAsFunctionCall);
    verifyNoMoreInteractions(constructor1);
    verifyNoMoreInteractions(constructor2);
  });
